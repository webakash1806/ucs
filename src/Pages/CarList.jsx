            ...
            <div className='flex flex-col py-10  px-[5vw] sm:px-[7vw] md:px-[9vw] lg:px-[11vw] items-center justify-center gap-4'>
                {
                    active === 1 ?
                        ...
                        filteredData && filteredData?.rates?.length === 0 ?
                            <p>No Cabs available to this city right now</p> :
                            filteredData?.rates?.map((data, index) => {
                                return <div className='flex items-start max-w-[60rem]  justify-between w-full gap-2 p-2 border rounded-md' key={index}>
                                    <img src={car1} className='w-[7rem]' alt="" />
                                    <div>
                                        <h2>{data?.category?.name}</h2>
                                        <p>{data?.perKm}/km</p>
                                        <p>{data?.perHour}/hr</p>
                                        <p>{data?.rateFor120Km12Hours}</p>
                                    </div>
                                </div>
                            })
